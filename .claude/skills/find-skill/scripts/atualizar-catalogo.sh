#!/bin/bash
# atualizar-catalogo.sh
# Reconstrói o catálogo de skills a partir das 14 fontes.
# Adaptado de fockus/claude-skill-find-skill (MIT) — update-skills-catalogue.sh.
# Mudanças: cache em diretório gravável configurável (FIND_SKILL_CACHE) em vez
# de caminho fixo, e .env configurável (FIND_SKILL_ENV).
...
# Cache gravável: a pasta da skill pode ser somente leitura (skill de conta
# sincronizada), então o catálogo atualizado vai para ~/.claude/skills/find-skill.
CACHE_DIR="${FIND_SKILL_CACHE:-$HOME/.claude/skills/find-skill}"
CATALOGUE="$CACHE_DIR/catalogue.json"
LOG="$CACHE_DIR/update.log"
ENV_FILE="${FIND_SKILL_ENV:-$CACHE_DIR/.env}"
...
if [ -z "${SKILLSMP_API_KEY:-}" ]; then
    echo "⚠️  SKILLSMP_API_KEY não definida. Adicione em $ENV_FILE" | tee -a "${LOG:-/dev/null}"
    echo "   Rode: echo 'export SKILLSMP_API_KEY=\"sua_chave\"' >> $ENV_FILE" | tee -a "${LOG:-/dev/null}"
fi
